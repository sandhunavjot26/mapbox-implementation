...
/**
 * Full Figma detections experience (node 1442:15801) — standalone route / embed.
...
function FooterPills({ children }: { children: ReactNode }) {
  return (
    <div className="flex flex-wrap items-start gap-2" style={{ gap: "8px" }}>
      {children}
    </div>
...
function ListRowShell({ children }: { children: ReactNode }) {
  return (
    <div
...
export function OverallDetectionPanel({
  variant = "overlay",
}: {
  /** `page`: full route with app chrome; `overlay`: floating panel on dashboard */
  variant?: "overlay" | "page";
} = {}) {
  const titleId = "overall-detections-panel-title";
  ...
  return (
    <div
      ...
      aria-modal="false"
      className={`driif-mission-scrollbar flex flex-col overflow-y-auto ${maxHClass}`}
      style={{
        width: PANEL_W,
