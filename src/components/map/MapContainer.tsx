...
// Initial map center (Jammu region) — fly-in destination
...
export function MapContainer({ mapMode }: MapContainerProps) {
  ...
  useEffect(() => {
    const tryInitialize = () => {
      ...
      // 1️⃣ Map initialization — global view, satellite, no pitch yet
      ...
      map.on("load", () => {
        ...
        // 4️⃣ After fly-in ends → operational mode: add layers and attach UI listeners
        map.once("moveend", () => {
  ...
  // 2D / 3D toggle (terrain already added in load; this only adjusts pitch/terrain on/off)
  useEffect(() => {
